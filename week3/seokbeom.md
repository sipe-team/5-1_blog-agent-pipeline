...
- Stop Hook의 1000자 제한 및 알려진 Hook 미발화 버그로 인한 데이터 유실 확인
...
| **Hook 미발화 버그** | Skill 내부에서 Stop Hook이 발화되지 않는 알려진 버그([#19225](https://github.com/anthropics/claude-code/issues/19225)), Subagent 완료 시 Stop Hook 미발화([#33049](https://github.com/anthropics/claude-code/issues/33049)) 등으로 특정 상황에서 기록 자체가 누락됨 |
...
## 참고 — Claude Code Agent Teams

이번 주 조사 과정에서 Agent Teams의 동작 방식을 확인했다. 향후 파이프라인 확장 시 활용 가능성이 있어 정리.

### 동작 구조

- **Lead** (메인 세션) — 전체 작업 설계, 지시, 결과 취합
- **Teammate들** (독립 세션) — Lead가 지정한 파트를 각자 병렬 작업

예: "블로그 서비스 만들어줘" 요청 시
1. Lead가 작업을 분할 (로그인/인증, 블로그 CRUD, 알림 등)
2. 각 Teammate가 독립 컨텍스트(1M 토큰)에서 병렬 작업
3. 메일박스 시스템으로 Lead에게 결과 보고
4. Lead가 취합·조율

### 주요 특징

- 각 Teammate는 **독립된 컨텍스트 윈도우**를 가짐 (대화 이력 공유 안 함)
- 같은 repo 워킹 디렉토리에서 각자 파일을 만들고 수정하는 방식 (자동 git merge가 아님)
- 토큰은 Teammate 수만큼 **선형 증가**
- 현재 실험 기능(experimental) — 기본 비활성 상태
