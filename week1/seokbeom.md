...
- 입력을 의미 단위로 분해합니다.
- 각 세그먼트에 카테고리와 보조 라벨을 부여합니다.
- `hold / useful / discard` 후보를 판단합니다.

예시 지시문:

```text
주어진 raw 입력을 의미 단위로 분해하고, 각 세그먼트에 대해 다음을 판단하라.
1. 상위 카테고리 (tech_issue / work_update / idea / reflection / todo / noise)
2. 보조 토픽
3. 정보 밀도 (high / medium / low)
4. 즉시 반영 여부 (useful / hold / discard)
원문을 과도하게 해석하지 말고, 명시된 내용 위주로 태깅하라.
```

### 2단계 - Category-specific Formatter

- 기술/코드: 문제 / 원인 가설 / 시도 / 결과 / 다음 액션 구조로 정리합니다.
- 회고/일기: 상태 / 맥락 / 감정 / 생각 / 후속 관찰 포인트 구조로 정리합니다.
- 아이디어: 문제의식 / 배경 / 가설 / 확장 포인트 / 다음 실험 구조로 정리합니다.

### 3단계 - Weekly Publisher

- 기술 카테고리 산출물 중 블로그 가치가 있는 항목을 선별합니다.
- 회고 카테고리 산출물 중 흐름이 있는 항목만 발행 후보로 승격합니다.
- 민감 정보는 보수적으로 필터링합니다.
